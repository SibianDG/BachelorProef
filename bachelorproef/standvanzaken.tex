...
Er zal dus eerst beschreven worden wat \textit{elderspeak} precies is. 
Waarom ergeren ouderen zich hieraan? 
% TODO: Extra stukken
Wat zijn de eigenschappen en hoe kan je het voorkomen?
...
\subsection{Kenmerken van \textit{elderspeak}
...
\subsection{Tips om \textit{elderspeak} te voorkomen?}
...
Een voorbeeld van clustering is marktsegmentatie waarbij klanten worden opgedeeld in verschillende segmenten zoals trouwe klant, mogelijke vertrekkende klant, ontevreden klant etc. Op basis van aankoopdata, tijdstippen en online activiteit kan men de klanten indelen in clusters. Die informatie kan dan weer gebruikt worden om mensen die behoren tot bepaalde clusters korting te geven of op te bellen waarom ze bijvoorbeeld niet tevreden zijn.
Daarnaast zijn er algoritmen die fraude opsporen op een website die niet normaal gedrag detecteren. Dit is dan een voorbeeld van anomalie detectie.
Bij PCA wordt de data gereduceerd zodat de minder relevante data weggefilterd wordt in de dataset.

\subsubsection{Reinforcement learning}
\textcite{Lievens2021} beschrijft leren door bekrachtiging door een techniek die niet echt een dataset gebruikt, maar eerder via een identiteit die leeft in een (on)bekende wereld. Die identiteit krijgt dan beloningssignalen. De opdracht is dan om uit te zoeken welke regels leiden tot een grote beloning.
