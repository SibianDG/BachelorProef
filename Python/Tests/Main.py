...
for person in range(1, 10):
    ...
    r = re.compile(f"{p}_sub[0-9]_1_[0-1]_[0-1]\.mp3")
    newlist1 = list(filter(r.match, arr))
    r = re.compile(f"{p}_sub[0-9]_[0-1]_1_[0-1]\.mp3")
    newlist2 = list(filter(r.match, arr))
    r = re.compile(f"{p}_sub[0-9]_[0-1]_[0-1]_1\.mp3")
    newlist3 = list(filter(r.match, arr))
    s = set(newlist1 + newlist2 + newlist3)
    print("SET:")
    print(s)
    for f in s:
        multipart_form_data = {
